...
export default function NavBar({ links }: { links: Link[] }) {
  ...
  return (
    <header
      className={`fixed w-full flex justify-between items-center z-10 backdrop-blur-sm px-4 py-2 bg-transparent shadow-2xl ${toggle &&
    ...
    >
      ...
      {links.length > 0 && (
        <>
          <nav
            className={`fixed w-full left-0 -top-[100vh] duration-1000 md:static md:w-auto md:duration-0 md:visible ${toggle
          ...
          >
            <ul
              className={`mb-0 md:pl-0 flex flex-wrap justify-between items-center ${toggle && 'active'
                }`}
            >
          ...
          <div
            className={`toggle md:hidden md:pointer-events-none w-[65px] h-[65px] relative mt-[10px] caret-transparent cursor-pointer rotate-0 transition ease-in-out duration-500 motion-reduce:transition-none ${toggle && 'open'
              } `}
