...
import HackerPackage from '@email/hacker-package';
...
const generateQRCodeOptions = (code: string): Partial<Options> => {
  return {
    width: 400,
    height: 400,
    margin: 10,
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let email of emails) {
      ...
      // Check if participant is not in REVIEW
      if (participant.status === ParticipantStatus.REVIEW) {
        status[email] = 'REVIEW';
        continue;
      }
      ...
      if (
        ...
        status[email] = 'NOT RECEIVED ACCEPTANCE';
        continue;
      }

      // Check if participant has already recieved
      if (
        participant.status === ParticipantStatus.EMAILED_PACKAGE &&
        participant.emailStatus?.get('hacker-package') === EmailStatus.RECEIVED
      ) {
      ...
      const emailRequest = await transporter.sendMail({
        ...
        subject: `Get Ready to Hack at DS3 Datathon 2024, ${participant.firstName} ${participant.lastName}!`,
        html: emailHTML,
        attachments: [
          {
            filename: 'DS3Id.jpeg',
